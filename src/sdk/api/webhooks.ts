...
import * as w from "../types/webhooks.ts";
import { RequesterClass } from "../types/requester.ts";
import { WAConfigType } from "../types/config.ts";
import { WAConfigEnum } from "../types/enums.ts";
import { generateXHub256Sig } from "../utils.ts";
            ...
            (req: IncomingMessage, res: ServerResponse) => {
                ...
                if (req.url) {
                    ...
                    if (
                        ...
                        if (req.method === "GET") {
                            if (
                                requestPath.searchParams.get("hub.mode") ==
                                    "subscribe" &&
                                requestPath.searchParams.get(
                                        "hub.verify_token",
                                    ) ==
                                    this.config[
                                ...
                                const errorMessage =
                                    `webhook subscription request from ${requestPath.href} has either missing or non-matching verify token`;
                            ...
                            req.on("end", () => {
                                ...
                                const cbBody: w.WebhookObject = JSON.parse(
                                    body,
                                );
                                ...
                                if (generatedSignature == xHubSignature) {
                                    ...
                                    const errorMessage =
                                        "error: x-hub signature doesn't match";
                                    const responseStatus = 401;
